...
def regress_vectors(x: List[float], y: List[float]) -> Optional[Tuple[float, float]]:
...
def residuals_points(points: Points, alpha: float, beta: float) -> List[float]:
    res = []
    for i in range(len(points)):
