...
def integrate(poly: Polynomial) -> Polynomial:
    ...
    if len(poly) != 1 or poly[0] != Fraction(0):
        result.append(Fraction(0))
    return result
...
def check_inverse(poly: Polynomial) -> bool:
    dif_int = differentiate(integrate(poly)) == poly
    int_dif = all([poly[i] == integrate(differentiate(poly))[i] for i in range(len(poly)-1)])
    return dif_int and int_dif
