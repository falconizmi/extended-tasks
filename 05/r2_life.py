...
def cell_value(grid: Matrix, x: int, y: int) -> int:
    if 0 <= x < len(grid) and 0 <= y < len(grid):
        return grid[x][y]
    return 0


def live_neighbour_count(grid: Matrix, x: int, y: int) -> int:
...
def next_value(grid: Matrix, x: int, y: int) -> int:
    assert x < len(grid) and y < len(grid)
...
def life(grid: Matrix, count: int) -> Matrix:
    assert len(grid) > 0
    assert count >= 0
