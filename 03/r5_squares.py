...
def least_squares(x, y):
    ...
    vertical_x = all([x[0] == x[i] for i in range(len(x))])
    if vertical_x:
        return None

    beta_sum = sum([(x[i]-average_x)*(y[i]-average_y) for i in range(len(x))])
    beta_x = sum((x[i]-average_x) ** 2 for i in range(len(x)))
    beta = beta_sum/beta_x
    alfa = average_y - beta*average_x
    r = [abs(y[i] -(alfa + beta*x[i])) for i in range(len(x))]
