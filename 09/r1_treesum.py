...
def sum_tree(tree: Tree) -> int:
    first = 0 if tree.first is None else sum_tree(tree.first)
    second = 0 if tree.second is None else sum_tree(tree.second)
    third = 0 if tree.third is None else sum_tree(tree.third)
    return tree.value + first + second + third
