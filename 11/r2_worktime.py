...
def free_slots(calendar: Calendar, day: date) -> List[Slot]:
    ...
    result = []
    plans_on_day = sorted([slot for slot in calendar if slot[0].day <= day.day <= slot[1].day])
    start = datetime.combine(day, time(8, 0))
    end = datetime.combine(day, time(17, 0))

    if len(plans_on_day) == 0:
        result.append((start, end))
        return result

    for slot in plans_on_day:
        if slot[0].timestamp() - start.timestamp() > 0:
            result.append((start, slot[0]))

        if slot[1].timestamp() - start.timestamp() > 0:
            start = slot[1]

        if end.timestamp() - slot[1].timestamp()  <= 0:
            start = end
            break
