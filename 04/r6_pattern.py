...
def find_pattern(text, pattern):
    ...
    for i, elem in enumerate(text):
        ...
        shift = next_pattern(idx, pattern, elem)
        if shift != -1:
            idx += shift
        else:
            shift = next_pattern(0, pattern, elem)
            if shift != -1:
                first_pattern = idx
                idx += shift
...
def next_pattern(idx, pattern, elem):
    if pattern[idx] == "[" and pattern[idx + 1] <= elem and elem <= pattern[idx + 3]:
        return 5
    if pattern[idx] == "?":
        return 1
    if pattern[idx] == elem:
...
def split_pattern(pattern):
    ...
    while i < len(pattern):
        if pattern[i] == "[":
            result.append((pattern[i+1], pattern[i+3]))
            i += 5
        else:
