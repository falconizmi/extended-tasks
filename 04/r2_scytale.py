...
def scytale_encrypt(text, circumference):
    ...
    columns = ceil(len(no_space)/ circumference)
    for i in range(columns):
        result += no_space[i::columns] + " "*(circumference - len(no_space[i::columns]))
    return result.rstrip()


def scytale_decrypt(text, circumference):
    result = "".join([text[i::circumference] for i in range(circumference)])
    return result.rstrip()
